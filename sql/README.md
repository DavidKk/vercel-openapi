...
Used by `/api/finance/stock/tasi/*` and cron sync tasi-sync. **Data flow:** today is read from KV snapshot (if not expired) or remote; DB is backup only. New trading days are written to DB then KV snapshot. See **services/finance/tasi/README.md** for full scheme (KV expiry, write rules, cron as fallback).
...
- Environment: same Turso as rest of app (`TURSO_DATABASE_URL`, `TURSO_AUTH_TOKEN`), plus `SAHMK_API_KEY` for today's data and cron.
