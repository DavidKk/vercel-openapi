...
import { createContext, type ReactNode, type RefObject, useCallback, useContext, useEffect, useId, useMemo, useRef, useState } from 'react'
...
interface ModuleNavContextValue {
  /** Current module sidebar items (empty outside module layouts). */
  items: DashboardSidebarItem[]
  /** Register items while a module layout is mounted. */
  setItems: (items: DashboardSidebarItem[]) => void
  open: boolean
  openDrawer: () => void
  closeDrawer: () => void
  toggleDrawer: () => void
  menuButtonRef: RefObject<HTMLButtonElement | null>
  /** ID of the mobile drawer panel for `aria-controls`. */
  drawerId: string
}

const ModuleNavContext = createContext<ModuleNavContextValue | null>(null)

/**
 * Access module mobile nav state (registrar, menu button, drawer).
 * @returns Context value
 */
function useModuleNavContext(): ModuleNavContextValue {
  const ctx = useContext(ModuleNavContext)
  if (!ctx) {
    throw new Error('useModuleNavContext must be used within ModuleNavProvider')
  }
  return ctx
}

interface ModuleNavProviderProps {
  children: ReactNode
}

/**
 * Provides module sub-nav state and renders the mobile drawer portal.
 * @param props Provider children
 * @returns Provider wrapper
 */
export function ModuleNavProvider(props: Readonly<ModuleNavProviderProps>) {
  const { children } = props
  const pathname = usePathname() ?? ''
  const [items, setItems] = useState<DashboardSidebarItem[]>([])
  ...
  const value = useMemo(
    ...
    [items, open, openDrawer, closeDrawer, toggleDrawer, drawerId]
  )
  ...
  const drawerAriaLabel = activeItem ? `${activeItem.title} — module menu` : 'Module menu'

  return (
    <ModuleNavContext.Provider value={value}>
      {children}
      {mounted && visible && items.length > 0
        ? createPortal(
            <div
...
interface ModuleNavRegistrarProps {
  /** Sidebar items for the active module layout. */
  items: DashboardSidebarItem[]
}

/**
 * Registers module sidebar items for the global mobile menu button (clears on unmount).
 * @param props Registrar props
 * @returns null
 */
export function ModuleNavRegistrar(props: Readonly<ModuleNavRegistrarProps>) {
  const { items } = props
  const { setItems } = useModuleNavContext()
...
/**
 * Mobile-only module menu trigger for the global app header (left of logo).
 * @returns Menu button or null when no module items are registered
 */
export function ModuleNavMenuButton() {
  const pathname = usePathname() ?? ''
  const { items, open, toggleDrawer, menuButtonRef, drawerId } = useModuleNavContext()
  ...
  const activeItem = findActiveDashboardSidebarItem(pathname, items)
  const label = activeItem ? `Open ${activeItem.title} module menu` : 'Open module menu'

  return (
    <button
      ref={menuButtonRef}
      type="button"
      className={`flex h-9 w-9 shrink-0 items-center justify-center rounded-lg text-gray-700 transition-[background-color,transform,box-shadow] duration-200 ease-out hover:bg-gray-100 motion-reduce:transition-none md:hidden ${
        open ? 'scale-95 bg-gray-100 shadow-inner' : 'scale-100'
      }`}
      aria-expanded={open}
      aria-controls={drawerId}
      aria-label={open ? 'Close module menu' : label}
