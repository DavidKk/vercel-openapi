...
type FuelToolName = 'get_fuel_price' | 'get_fuel_price_by_province' | 'calc_fuel_recharge_promo'

interface FuelPriceMcpState {
  loading: boolean
  status?: number
  durationMs?: number
  error?: string
...
export function FuelPriceMcpPlayground() {
  ...
  function handleToolChange(value: string) {
    const tool = value as FuelToolName
    let nextParams = '{}'
    if (tool === 'get_fuel_price_by_province') {
      nextParams = JSON.stringify({ province: '北京' }, null, 2)
    } else if (tool === 'calc_fuel_recharge_promo') {
      nextParams = JSON.stringify({ amount: 200, province: '北京' }, null, 2)
    }
    setState((prev) => ({
      ...prev,
      tool,
          ...
          <div className="space-y-2 px-3 py-2 text-[11px] text-gray-700">
            <label className="flex flex-col gap-1 text-[11px]">
              ...
              <FormSelect
                ...
                options={[
                  { value: 'get_fuel_price', label: 'get_fuel_price' },
                  { value: 'get_fuel_price_by_province', label: 'get_fuel_price_by_province' },
                  { value: 'calc_fuel_recharge_promo', label: 'calc_fuel_recharge_promo' },
                ]}
              />
            </label>
            <label className="flex flex-col gap-1 text-[11px]">
