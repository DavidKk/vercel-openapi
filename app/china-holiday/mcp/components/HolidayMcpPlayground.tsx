...
type HolidayToolName = 'get_today_holiday' | 'list_holiday' | 'is_workday' | 'is_holiday'

interface HolidayMcpState {
  loading: boolean
  status?: number
  durationMs?: number
  error?: string
  responseBody?: string
  tool: HolidayToolName
  paramsText: string
}

/** MCP endpoint for this module (POST /api/mcp/holiday) */
const MCP_PATH = '/api/mcp/holiday'

/**
 * Client-side playground for holiday-related MCP tools.
 */
export function HolidayMcpPlayground() {
  const [state, setState] = useState<HolidayMcpState>({
    loading: false,
  ...
  function handleToolChange(value: string) {
    const tool = value as HolidayToolName
    let nextParams = '{}'
    if (tool === 'list_holiday') {
      nextParams = JSON.stringify({ year: new Date().getFullYear() }, null, 2)
    } else if (tool === 'is_workday' || tool === 'is_holiday') {
      nextParams = JSON.stringify({ date: new Date().toISOString().slice(0, 10) }, null, 2)
    }
    setState((prev) => ({
      ...prev,
      tool,
      paramsText: nextParams,
    }))
  }

  function handleParamsChange(event: React.ChangeEvent<HTMLTextAreaElement>) {
          ...
          <div className="flex flex-col gap-2 px-3 py-2 text-[11px] text-gray-700">
            <label className="flex flex-col gap-1 text-[11px]">
              ...
              <FormSelect
                ...
                options={[
                  { value: 'get_today_holiday', label: 'get_today_holiday' },
                  { value: 'list_holiday', label: 'list_holiday' },
                  { value: 'is_workday', label: 'is_workday' },
                  { value: 'is_holiday', label: 'is_holiday' },
                ]}
              />
            </label>
            <label className="flex flex-col gap-1 text-[11px]">
        ...
        <div className="flex min-h-0 flex-1 flex-col bg-white">
          ...
          <div className="min-h-0 flex-1 overflow-auto bg-white p-2 text-[10px] leading-relaxed text-gray-800">
            {error ? (
              ...
              <span className="text-gray-500">No response yet. Choose a tool and send a request to see the result.</span>
            )}
          </div>
        </div>
      </form>
