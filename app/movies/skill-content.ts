...
## MCP tool: list_latest_movies

The same data is also exposed as an MCP tool:

  name: list_latest_movies
  description: List latest movies from cache (Maoyan + TMDB merge). Returns { movies, cachedAt }. Optional year filter.

Parameters:
- year (optional, number): Filter by release year (e.g. 2025).

Example tool call (JSON):
  {
    "tool": "list_latest_movies",
    "params": { "year": 2025 }
  }
...
Agents can either:
- Call HTTP API directly: GET BASE_URL/api/movies
- Or, in an MCP-enabled environment, prefer the \`list_latest_movies\` tool for consistent access control and logging.
`
