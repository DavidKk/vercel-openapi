...
Configure `RSSHUB_BASE_URL` for your RSSHub instance. Source paths live in `services/news/news-sources.manifest.ts` (`newsSourcesManifest`). Rolling window length is **per list slug** (see `services/news/published-recent-window.ts`); env **`NEWS_FEED_RECENT_HOURS`** applies only to the **no-list / no-category** merged pool.
