...
import { invalidParameters, jsonForbidden, jsonSuccess } from '@/initializer/response'
...
export const GET = api(async (_req, context) => {
  ...
  if (id) {
    try {
      ...
      if (!product) {
        return invalidParameters('product not found').toJsonResponse(404)
      }
      return jsonSuccess(product)
    } catch (error) {
      if (isNotAuthorizedError(error)) return jsonForbidden()
      const message = error instanceof Error ? error.message : 'Failed to fetch product'
      return invalidParameters(message).toJsonResponse(400)
    }
  }
...
export const POST = api(async (req) => {
  ...
  if (!name || !unit || unitBestPrice === undefined) {
    return invalidParameters('invalid product payload').toJsonResponse(400)
  }
...
export const PUT = api(async (req, context) => {
  const id = context.searchParams.get('id') ?? ''
  if (!id) {
    return invalidParameters('missing id').toJsonResponse(400)
  }
  ...
  if (Object.keys(updates).length === 0) {
    return invalidParameters('invalid product updates').toJsonResponse(400)
  }
