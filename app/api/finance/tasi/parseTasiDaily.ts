...
export function parseTasiCompaniesDaily(html: string): TasiCompanyDailyRecord[] {
  ...
  try {
    const lastUpdateText = $('.lastUpdate').first().text().trim()
  ...
  $('table').each((_: any, table: any) => {
    ...
    if (!headers.length) {
      ...
      headers = firstRow
        .find('td,th')
        .toArray()
        .map((h: any) => $(h).text().trim().toLowerCase())
    }
    ...
    const rowCount = dataRows.length ? dataRows.length : Math.max(0, $table.find('tr').length - 1)
  ...
  if (!dataTable) {
    ...
    if (first.length) dataTable = first
  }
  ...
  const headerCells = dataTable.find('thead th, thead td')
  let headersText: string[] = []
  if (headerCells.length) {
    headersText = headerCells.toArray().map((h: any) =>
      $(h)
        .text()
        .trim()
        .toLowerCase()
    ...
    const firstRow = dataTable.find('tr').first()
    headersText = firstRow
      .find('td,th')
      .toArray()
      .map((h: any) =>
        $(h)
