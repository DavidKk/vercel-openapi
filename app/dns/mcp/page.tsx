...
import { DnsMcpPlayground } from './components'

/**
 * DNS Query MCP page.
 * Left: MCP tools doc. Right: playground for POST /api/mcp/dns (dns_query tool).
 */
export default function DnsMcpPage() {
  return (
    <div className="flex h-full flex-nowrap overflow-x-auto overscroll-x-contain md:overflow-visible">
      <section className="flex h-full min-h-0 flex-shrink-0 w-[85vw] min-w-[280px] flex-col border-r border-gray-200 bg-white md:w-1/2 md:min-w-[320px] md:flex-1">
        <DocPanelHeader title="DNS Query MCP tools" subtitle="Tools for DNS lookup via the MCP server." />
        <div className="min-h-0 flex-1 overflow-y-auto px-3 py-2 text-[11px] text-gray-800">
          <McpOneClickInstallBar endpointPath="/api/mcp/dns" className="mb-3" />
          <h2 className={DOC_SECTION_TITLE_CLASS}>Endpoints</h2>
          <div className={DOC_ENDPOINT_BOX_CLASS}>
            <DocEndpointRow method="POST" path="/api/mcp/dns" />
            <p className={DOC_ENDPOINT_DESC_CLASS}>
              Body: <code className="rounded bg-gray-100 px-1 py-0.5 text-[10px]">{'{ tool, params }'}</code>. Call{' '}
              <code className="rounded bg-gray-100 px-1 py-0.5 text-[10px]">dns_query</code> with domain (and optional dns).
            </p>
          </div>
          <h2 className={DOC_SECTION_TITLE_CLASS}>Available tools</h2>
          <ul className="mb-3 list-disc pl-4">
            <li className="mb-1">
              <code className="rounded bg-gray-100 px-1 py-0.5 text-[10px]">dns_query</code> – Query DNS for a domain (A and AAAA records). Params: domain, dns (optional, default
              1.1.1.1).
            </li>
          </ul>
        </div>
