...
import { CHINA_WEATHER_DEMO_LOCATION } from '@/app/china-weather/lib/china-weather-demo-location'
...
type WeatherToolName = 'get_point_weather' | 'get_point_forecast'

interface WeatherMcpState {
  loading: boolean
  status?: number
  durationMs?: number
  error?: string
...
/** MCP endpoint for this module (POST /api/mcp/weather) */
...
export function WeatherMcpPlayground() {
  ...
  function handleToolChange(value: string) {
    const tool = value as WeatherToolName
          ...
          <div className="flex flex-col gap-2 px-3 py-2 text-[11px] text-gray-700">
            <label className="flex flex-col gap-1 text-[11px]">
              ...
              <FormSelect
                ...
                options={[
                  { value: 'get_point_weather', label: 'get_point_weather' },
                  { value: 'get_point_forecast', label: 'get_point_forecast' },
                ]}
              />
            </label>
            <label className="flex flex-col gap-1 text-[11px]">
