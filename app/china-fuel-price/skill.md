...
- Triggers: phrases about **fuel price** / **gasoline grade + province** / **Guangdong (or other province) price** / **recharge promo** (any language).
...
- **Single province:** need **province** path segment (format accepted by the API, often locale-specific or English). **Parse first**; if missing, ask once.
...
- User: “Today’s fuel price in Beijing” → `GET /api/fuel-price/Beijing` (or matching slug) → summarize `current`.
- User: “Fuel prices nationwide” → `GET /api/fuel-price`.
