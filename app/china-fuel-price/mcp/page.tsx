...
/**
 * China Fuel Price MCP tools page.
 * Left side documents fuel-related MCP tools, right side is a playground for POST /api/mcp/fuel-price.
 * @returns China Fuel Price MCP tools page
 */
  ...
  return (
    <div className="flex h-full flex-nowrap overflow-x-auto overscroll-x-contain md:overflow-visible">
      ...
      <section className="flex h-full min-h-0 flex-shrink-0 w-[85vw] min-w-[280px] flex-col border-r border-gray-200 bg-white md:w-1/2 md:min-w-[320px] md:flex-1">
        <DocPanelHeader title="China Fuel Price MCP tools" subtitle="Tools exposed through the MCP server for querying fuel prices and calculating recharge promotions." />
        <div className="min-h-0 flex-1 overflow-y-auto px-3 py-2 text-[11px] text-gray-800">
          <McpOneClickInstallBar endpointPath="/api/mcp/fuel-price" className="mb-3" />
          <h2 className={DOC_SECTION_TITLE_CLASS}>Endpoints</h2>
          <div className={DOC_ENDPOINT_BOX_CLASS}>
            <DocEndpointRow method="POST" path="/api/mcp/fuel-price" />
