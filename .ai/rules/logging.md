...
4. **What to log**
   - **info:** Request or operation start; success with minimal context (e.g. `{ date, count }`, `{ province }`). Do not log secrets or full request bodies.
...
- Logger implementation: `services/logger/index.ts` — `createLogger(moduleName)` returns `{ info, warn, fail }`.
