...
| `RSSHUB_CF_ACCESS_CLIENT_ID`     | No       | With `RSSHUB_CF_ACCESS_CLIENT_SECRET`, sent as `CF-Access-Client-Id` on News RSS fetches (Cloudflare Access service token when RSSHub is behind Zero Trust).                                                                                                    |
| `RSSHUB_CF_ACCESS_CLIENT_SECRET` | No       | With `RSSHUB_CF_ACCESS_CLIENT_ID`, sent as `CF-Access-Client-Secret`. Omit either variable to disable (both required).                                                                                                                                          |
