...
- **UX:** warmup `retrySourceIds` is **silent** (no top skeleton, no sidebar “warming” copy, no enter animation on merged rows). The first full-page network refresh still may show the small head skeleton while L0 rows are visible (`getNewsOverviewHeadRefreshVisible`). React **`key`** on rows uses **`getNewsFeedItemListKey`**, not array index, to limit unnecessary reordering. Successful silent merge updates L0 when the URL has **no** facet (`shouldPersistNewsOverviewL0`).
